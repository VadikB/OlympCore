...
template <typename T>
struct quantile_inc_t : public Sort::operator_temp_copy_t<T>
{
    TCSize_t q;
    quantile_inc_t(TCSize_t q)
    {
        this->q = q;
    }
    inline TCErrorCode GetResult(T_state &state, TCSize_t nelem, T *output, TCSize_t output_stride)
    {
        ...
        for (TCSize_t i = 0; i <= q; i++)
        {
            ...
            else
            {
                if (m == nelem - 1)
                {
                    v = quick_select<T, false, TCSize_t, interpolation_t<T,1,2>>(data, nelem, m);
                }
                else
                {
                    interpolation_with_d_t<T> mean(d);
                    v = quick_select<T, true, TCSize_t>(data, nelem, m, mean);
                }
            }
            output[i*output_stride] = v;
        }
        return tcerror_code_new(TCError_NoError, TCArgPosition_UnKnown);
    }
};
