...
namespace Smoothing {
		...
		T* external_data_lag, TCUInt64 lag_size, T* external_data_lead, TCUInt64 lead_size, TCUInt64 n, T* tma) 
	{
		...
		{
			*tma = _get_tc_error<T>(TCError_IllegalInput); //NaN in case previous data is undefined
			return;
		}
		...
		const TCArray* external_data_lag, const TCArray* external_data_lead, TCArray* tma_series) 
	{
		...
		//Check: Window length is less than or equal to zero. type is unsigned
		if (window_length == 0)
		{
		...
		{
			//Check: ±Inf in the offset
			//Check: Error NaN in the offset
			//Check: Offset is negative value
		...
		//Check: another checks are in calculation
