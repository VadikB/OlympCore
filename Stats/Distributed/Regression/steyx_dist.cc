...
template<typename T>
TCErrorCode math_desc_steyx(TCDistArray *known_ys, TCDistArray *known_xs, TCDistArray *output)
{
    std::vector<T> tmpMem;
    ...
    reduce2arrays_op(sum2sqr_kahan_t<T>(), y_local->m_numelt, 
        (T*)y_local->m_data, (T*)x_local->m_data, 
        mean_y, tc_static_cast<T>(0), tc_static_cast<T>(0)/*value_init*/, &yy,
        (MPI_Comm)known_ys->m_Layout.m_Comm, MASTER_RANK, MPI_SUM);
    ...
    if (rank == MASTER_RANK)
    {
        ...
        ret = Utils::sqrt(Utils::abs(yy-xy*xy/xx)/(count-tc_static_cast<T>(2)));
...
TCErrorCode math_desc_steyx_d_e_dist(TCDistArray *known_ys, TCDistArray *known_xs, TCDistArray *output)
{
    return math_desc_steyx<TCDouble>(known_ys, known_xs, output);
}

TCErrorCode math_desc_steyx_f_e_dist(TCDistArray *known_ys, TCDistArray *known_xs, TCDistArray *output)
{
    return math_desc_steyx<TCFloat>(known_ys, known_xs, output);
}

TCErrorCode math_desc_steyx_z_e_dist(TCDistArray *known_ys, TCDistArray *known_xs, TCDistArray *output)
{
    return math_desc_steyx<TCDComplex>(known_ys, known_xs, output);
}

TCErrorCode math_desc_steyx_c_e_dist(TCDistArray *known_ys, TCDistArray *known_xs, TCDistArray *output)
{
    return math_desc_steyx<TCFComplex>(known_ys, known_xs, output);
}
