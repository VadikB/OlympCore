...
template <typename T>
inline T small_dist(std::vector<T>& data, TCSize_t total_elem, TCSize_t k, MPI_Comm comm)
{
    return Sort::quick_select_dist<T, false, TCSize_t, interpolation_t<T,1,2>>(data, total_elem, k - 1, comm);
}

template <typename T>
struct small_dist_t : public small_t<T>, public Sort::operator_qselect_dist_t
{
    small_dist_t(TCSize_t k) : small_t(k)
    {
    }
    ...
    inline TCErrorCode GetResultDist(T_state& state, TCSize_t total_elem, MPI_Comm comm, T& result)
    {
        ...
        if (-1 == errorRankToUse)
        {
            result = small_dist<T>(state, total_elem, this->k, comm);
        }
