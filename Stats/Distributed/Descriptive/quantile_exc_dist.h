...
#include "TCConsts.h"
#include "../Descriptive/quantile_exc.h"
...
template <typename T>
struct quantile_exc_dist_t : public quantile_exc_t<T>
{
    ...
    quantile_exc_dist_t(TCSize_t q):quantile_exc_t(q){}
    ...
    inline void GetResultDist(T_state &state, std::vector<TCSize_t> &nelems, Sort::TCVectorIterator<T>* vit, TCSpan* span, MPI_Comm& comm)
    {
        ...
        for(TCSize_t ivector = 0; ivector < nvectors; ivector++)
        {
			...
			TCSize_t nexcess = p_excess * (q - 1);
			TCSize_t rborder = p_entier * (q - 1) + nexcess / q + 1;

			T_state::iterator itBegin = state.begin() + ivector * stride;
			T_state::iterator itEnd = itBegin + nelems[ivector];

			if(rborder < nelems[ivector])
			{
				T_state::iterator itMiddle = itBegin + rborder;

				std::partial_sort(itBegin, itMiddle, itEnd);
			}
			else
			{
        ...
        for (TCSize_t i = 1; i < q; i++)
        {
            ...
            for(TCSize_t ivector = 0; ivector < nvectors; ivector++)
            {
                ...
                if ((m[ivector] == 0) || (m[ivector] > gnelems[ivector]))
                {
            ...
            if((span->StartIdx < ui) && ((ui - 1) <= span->EndIdx))
            {
                TCSize_t elidx = i - span->StartIdx - 1;
                TCSize_t stride_low = vit->GetStride();
