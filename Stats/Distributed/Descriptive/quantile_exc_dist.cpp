...
#include "../Distributed/Descriptive/quantile_exc_dist.h"
#include "../Distributed/Descriptive/qselect_utils_dist.h"

using namespace Descriptive;

TCErrorCode stats_redop_quantile_exc_f_e_dist(const TCDistArray* numbers, TCSize_t narrays, TCSize_t dim, const TCSize_t q, TCDistArray** value)
{
    if (q < 2)
    {
    ...
    try
    {
        quantile_exc_dist_t<TCFloat> op(q);
...
TCErrorCode stats_redop_quantile_exc_d_e_dist(const TCDistArray* numbers, TCSize_t narrays, TCSize_t dim, const TCSize_t q, TCDistArray** value)
{
    if (q < 2)
    {
        return tcerror_code_new(TCError_IllegalSize, TCArgPosition_UnKnown);
    ...
    try
    {
        quantile_exc_dist_t<TCDouble> op(q);
        return Sort::process2vector_dist<Sort::SkipMissingValues, Sort::ConverMissingValuesTo0>(op, numbers, narrays, dim, value);
    }
    catch(std::bad_alloc &ba)
    {
