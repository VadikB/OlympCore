...
#define LOGNORM_METHOD VSL_RNG_METHOD_LOGNORMAL_BOXMULLER2_ACCURATE
...
template <typename T>
static int rand_ziggurat_lognorm(int method, TCRNGStatePtr state, int count, T* ptr, T mean, T stand_dev, T dummy1, T dummy2)
{
    const T r = 3.442620f;
    ...
    method; dummy1; dummy2; // unreferenced parameter
    ...
    for(int i = 0; i < count; i++)
    {
        ...
        ptr[i] = std::exp(x * stand_dev + mean);
    }
    return VSL_ERROR_OK;
}
...
TCErrorCode stats_lognorm_rand(const T mean, const T stand_dev, const TCArray* values, fcnType fcn)
{
    ...
    TCErrorCode result = stats_rand<T, tag, 3>(values, [mean, stand_dev, fcn](TCRNGStatePtr state, int count, T* ptr) -> TCErrorCode {
        int res = fcn(LOGNORM_METHOD, state, count, ptr, mean, stand_dev, 0, 1);
        if (res != VSL_ERROR_OK)
        {
...
template <typename T, TCTypeTag tag, typename fcnType>
TCErrorCode stats_lognorm_rand(const TCUInt64 seed, const T mean, const T stand_dev, const TCArray* values, fcnType fcn)
{
    if (isinf(mean) || isnan(mean))
    {
        return tcerror_code_new(TCError_IllegalInput, 2);
...
TCErrorCode stats_lognorm_rand_f_e(const TCFloat mean, const TCFloat stand_dev, TCArray* values)
{
    return stats_lognorm_rand<TCFloat, TC_FLOAT>(mean, stand_dev, values, rand_ziggurat_lognorm<TCFloat>);
}

TCErrorCode stats_lognorm_rand_d_e(const TCDouble mean, const TCDouble stand_dev, TCArray* values)
{
    return stats_lognorm_rand<TCDouble, TC_DOUBLE>(mean, stand_dev, values, rand_ziggurat_lognorm<TCDouble>);
}

TCErrorCode stats_lognorm_rand_seed_f_e(const TCUInt64 seed, const TCFloat mean, const TCFloat stand_dev, TCArray* values)
{
    return stats_lognorm_rand<TCFloat, TC_FLOAT>(seed, mean, stand_dev, values, rand_ziggurat_lognorm<TCFloat>);
}

TCErrorCode stats_lognorm_rand_seed_d_e(const TCUInt64 seed, const TCDouble mean, const TCDouble stand_dev, TCArray* values)
{
    return stats_lognorm_rand<TCDouble, TC_DOUBLE>(seed, mean, stand_dev, values, rand_ziggurat_lognorm<TCDouble>);
}