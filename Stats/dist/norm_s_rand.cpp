...
template <typename T>
static int rand_ziggurat_norm(int method, TCRNGStatePtr state, int count, T* ptr)
{
    const T r = 3.442620f;
...
TCErrorCode stats_norm_s_rand(const TCArray* values, fcnType fcn)
{
    TCErrorCode result = stats_rand<T, tag, 1>(values, [fcn](TCRNGStatePtr state, int count, T* ptr) -> TCErrorCode {
        int res = fcn(NORM_METHOD, state, count, ptr);
        if (res != VSL_ERROR_OK)
        {
...
template <typename T, TCTypeTag tag, typename fcnType>
TCErrorCode stats_norm_s_rand(const TCUInt64 seed, const TCArray* values, fcnType fcn)
{
    TCErrorCode result = stats_rand<T, tag, 2>(seed, values, [fcn](TCRNGStatePtr state, int count, T* ptr) -> TCErrorCode {
        int res = fcn(NORM_METHOD, state, count, ptr);
        if (res != VSL_ERROR_OK)
        {
            return tcerror_code_new(TCError_Internal, TC_UNSPECIFIED_ARGID);
...
TCErrorCode stats_norm_s_rand_f_e(TCArray* values)
{
    return stats_norm_s_rand<TCFloat, TC_FLOAT>(values, rand_ziggurat_norm<TCFloat>);
}

TCErrorCode stats_norm_s_rand_d_e(TCArray* values)
{
    return stats_norm_s_rand<TCDouble, TC_DOUBLE>(values, rand_ziggurat_norm<TCDouble>);
}

TCErrorCode stats_norm_s_rand_seed_f_e(const TCUInt64 seed, TCArray* values)
{
    return stats_norm_s_rand<TCFloat, TC_FLOAT>(seed, values, rand_ziggurat_norm<TCFloat>);
}

TCErrorCode stats_norm_s_rand_seed_d_e(const TCUInt64 seed, TCArray* values)
{
    return stats_norm_s_rand<TCDouble, TC_DOUBLE>(seed, values, rand_ziggurat_norm<TCDouble>);
}