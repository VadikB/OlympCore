...
template<typename T> 
TCErrorCode regression_slope_inner(TCSize_t numElements, TCBool , T* known_xs, T* known_ys, T* ret)
{
...
template<typename T> 
TCErrorCode  regression_slope(TCSize_t numElements, TCBool isref, T* known_xs, T* known_ys, TCSize_t rowsX, TCSize_t colsX, TCSize_t rowsY, TCSize_t colsY, T* ret)
{
    T *tmp = known_ys;
    std::vector<T> tmpMem;
    ...
    return regression_slope_inner(numElements, isref, known_xs, tmp, ret);
}
