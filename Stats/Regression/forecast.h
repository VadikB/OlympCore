...
template<typename T> 
TCErrorCode regression_forecast_inner(TCSize_t numElementsX, TCSize_t numElements, TCBool , T* x, T* known_xs, T* known_ys, T* ret)
{
    ...
    for(TCSize_t i=0;i<numElementsX;i++)
    {
        ret[i] = a + b * x[i];
    }
...
template<typename T> 
TCErrorCode  regression_forecast(TCSize_t numElementsX, TCSize_t numElements, TCBool isref, T* x, T* known_xs, T* known_ys, TCSize_t rowsX, TCSize_t colsX, TCSize_t rowsY, TCSize_t colsY, T* ret)
{
    T *tmp = known_ys;
    std::vector<T> tmpMem;
    ...
    return regression_forecast_inner(numElementsX, numElements, isref, x, known_xs, tmp, ret);
}
